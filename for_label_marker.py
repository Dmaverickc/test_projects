...
def csv_reader(file_obj):
	...
	for idx, row in enumerate(reader):       # goes through the rows of the csv
		...
		if row["name"] == prev_row:
			...
			#need to stop the lenght of odd_label 
			if len(odd_label) == 2:   # keeps the odd_labels list limited to one
			 ...
			 # limiting the len of odd_label to 2 is getting a value carried over or something, have to think of a new thing for it 02/01/2018 
...
def odd_printer(odd_filter, pos_filter):
	...
	if odd_filter not in pos_filter: 
		print odd_filter
